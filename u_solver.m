...
z = sym(zeros(n,3+n));% z = sym(z); %массив, в котором содержатся: 1 столбец - левая сторона уравнений, 2 столбец - правая сторона уравнений, 3 столбец - наличие в уравнении управления u
syms T u; 
er = 0;
while er < 1
...
if k > 0 %если счетчик k = 1, то цель задана по x_i по которому введено управление
    ...
    while i <= n
        i = i+1; 
        if i < n + 1 
            if strfind(string(psi), strcat('x',string(i))) > 0
                if strfind(string(z(i,2)),erase(string(x_u),'(t)')) > 0
                    m = m + 1;
                else 
                    x_fi_start = z(i,1);
                end
    ...
    if m > 0
        ...
        psi_1 = psi_1 - fi_1; %задаем fi для новой цели_1  
        sol_1 = subs(diff(psi_1), old, new); %замена в diff(psi) - diff(x_i(t), t) на правую часть x_i(t)
        EL_1 = simplify(T1*sol_1 + psi_1); %первое уравнение эйдера-лагранжа для поиска управдения
        disp('-------------------->');
        disp('u = ');
